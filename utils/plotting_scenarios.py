...
def plot_all_scenarios(DATADIR, FILE, FILENUM, RESULT_DIR, RESULT_FILENAME, RESULT_SOLO, FIGUREDIR):
    ...
    for actor_type, agents in actors_list.items():
        ...
        for agent in agents:
            ...
            _ = plot_solo_scenario(f"{actor_type}_{agent}", agent_activity, agent_interalation, agent_state, DATADIR,
                                   FILE, solo_scenario, agent_fig_path, \
                                   static_element, original_data_roadgragh, original_data_light)

    return 0


def plot_solo_scenario(agent, agent_activity, agent_interalation, agent_state, DATADIR, FILE, solo_scenario,
                       agent_fig_path, s_e, o_d_r, o_d_l):
    """
    ...
    ax_list1[0] = plot_actor_activity(agent_activity["long_v"], solo_scenario["lo"], \
                                      valid_start, valid_end, ax_list1[0], "Longitudinal velocity [m/s]",
                                      "Longitudinal activity [-]", "Longitudinal")
    # Plot longitudinal velocity and activity
    ax_list1[1] = plot_actor_activity(agent_activity["yaw_rate"], solo_scenario["la"], \
                                      valid_start, valid_end, ax_list1[1], "Yaw rate[rad/s]", "Lateral activity [-]",
                                      "Lateral")
    plt.tight_layout()
    plt.savefig(os.path.join(agent_fig_path, f"{agent}_activity.jpg"), bbox_inches="tight")
    plt.close()
    #################################
    ######  the second figure #######
    #################################
    ...
    ax_list2[0], _, _, _ = plot_road_graph(DATADIR, FILE, ax=ax_list2[0], static_element=s_e,
                                           original_data_roadgragh=o_d_r, original_data_light=o_d_l)
    # plot band relations type 2
    ax_list2[1], _, _, _ = plot_road_graph(DATADIR, FILE, ax=ax_list2[1], static_element=s_e,
                                           original_data_roadgragh=o_d_r, original_data_light=o_d_l)
    ...
    for key in agent_interalation:
        ...
        ax_list2[0] = plot_actor_polygons(guest_etp, guest_v_s, guest_v_e, ax_list2[0], f"ETP guest", gradient=False,
                                          host=False, type_a=False)
        ax_list2[1] = plot_actor_polygons(guest_ebb, guest_v_s, guest_v_e, ax_list2[1], f"EBB guset", gradient=False,
                                          host=False, type_a=False)
        ax_list2[0] = plot_actor_polygons(guest_trajectory_polygon, guest_v_s, guest_v_e, ax_list2[0], f"Actual guest",
                                          gradient=False, host=False, type_a=True)
        ax_list2[1] = plot_actor_polygons(guest_trajectory_polygon, guest_v_s, guest_v_e, ax_list2[1], f"Actual guest",
                                          gradient=False, host=False, type_a=True)
    ax_list2[0] = plot_actor_polygons(etp, valid_start, valid_end, ax_list2[0], f"ETP host", gradient=False, host=True,
                                      type_a=False)
    ax_list2[1] = plot_actor_polygons(ebb, valid_start, valid_end, ax_list2[1], f"EBB host", gradient=False, host=True,
                                      type_a=False)
    ax_list2[0] = plot_actor_polygons(actor_trajectory_polygon, valid_start, valid_end, ax_list2[0], f"Actual host",
                                      gradient=False, host=True, type_a=True)
    ax_list2[1] = plot_actor_polygons(actor_trajectory_polygon, valid_start, valid_end, ax_list2[1], f"Actual host",
                                      gradient=False, host=True, type_a=True)
    handels, labels = [], []
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(agent_fig_path, f"{agent}_inter_actor.jpg"), bbox_inches="tight")
    plt.close()
    #################################
    ######  the third figure #######
    #################################
    ...
    ax_list3[0], s_e, o_d_r, o_d_l = plot_road_graph(DATADIR, FILE, ax=ax_list3[0], static_element=s_e,
                                                     original_data_roadgragh=o_d_r, original_data_light=o_d_l)
    ax_list3[0] = plot_actor_polygons(actor_trajectory_polygon, valid_start, valid_end, ax_list3[0],
                                      "Actual trajectory", gradient=True, host=True, type_a=True)
    ...
    ax_list3[1], _, _, _ = plot_road_graph(DATADIR, FILE, ax=ax_list3[1], static_element=s_e,
                                           original_data_roadgragh=o_d_r, original_data_light=o_d_l)
    ax_list3[1] = plot_actor_polygons(actor_expanded_multipolygon, valid_start, valid_end, ax_list3[1],
                                      "Extended trajectory host", gradient=False, host=True, type_a=False)
    ax_list3[1] = plot_actor_polygons(actor_trajectory_polygon, valid_start, valid_end, ax_list3[1],
                                      "Actual trajectory host", gradient=False, host=True, type_a=True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(agent_fig_path, f"{agent}_roadgraph.jpg"), bbox_inches="tight")
    plt.close()
    return 0
...
def plot_actor_polygons(actor_polygon, valid_start: int, valid_end: int, ax, polygon_label: str, gradient: bool = False,
                        host: bool = True, type_a: bool = True, inter_actor: bool = False):
...
def plot_actor_activity(data, activity, valid_start, valid_end, ax1, legend_data: str, legend_activity: str,
                        title: str):
    ...
    ax1.plot(time[valid_start:valid_end + 1], data[valid_start:valid_end + 1], c='gray', linewidth=15, alpha=0.3,
             label='valid range')
...
def plot_actor_activity_2(data, activity, valid_start, valid_end, ax1, ax2, legend_data: str, legend_activity: str,
                          title: str):
    # sampling frequency = 10
    if not isinstance(valid_start, int):
        valid_start = int(valid_start)
    ...
    ax1.plot(time[valid_start:valid_end + 1], data[valid_start:valid_end + 1], c='gray', linewidth=15, alpha=0.3,
             label='valid range')
    ax2.plot(time[valid_start:valid_end + 1], activity[valid_start:valid_end + 1], c='gray', linewidth=15, alpha=0.3,
             label='valid range')
