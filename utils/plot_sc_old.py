...
DATA_DIR = Path("/home/manolotis/sandbox/datasets/waymo_v1.1/uncompressed/tf_example/testing")
...
def parse_json(RESULT_DIR):
    for sc_id in track(SC_ID,description="Plotting..."):
        ...
        for sc_file in SC_DIR.glob("*.json"):
            # print("sc_file.name", sc_file.name)
            # print("sc_file.name.split('_')", sc_file.name.split("_"))
            data_num = sc_file.name.split("_")[1]
            # ToDo: avoid hardcoding
            # DATA = f"validation_tfexample.tfrecord-{data_num}-of-01000"
            DATA = f"uncompressed_tf_example_validation_validation_tfexample.tfrecord-{data_num}-of-00150"
            try:
...
def plot_single_scenario(parsed, host:tuple, guest:tuple, time, sc_id, data_num, sc_num):
    ...
    FIG_PATH = ROOT / "figures" / "testing" / sc_id
