...
def plot_road_lines(ax,original_data_roadgragh:dict,original_data_light:dict,road_edge:bool=True,road_lines:bool=False,lane_center:bool=False,controlled_lane:bool=False, return_artists:bool=False):
    ...
    if road_edge:
        ...
        if (len(roadedge_pts[0,:])>0):
            artist = ax.scatter(roadedge_pts[0,:],roadedge_pts[1,:],color='k',marker='.',s=20,label="road edge points")
    ...
    if road_lines:
        ...
        if (len(roadline_pts[0,:])>0):
            artist = ax.scatter(roadline_pts[0,:],roadline_pts[1,:],color='k',marker='.',s=20,label="road line points")
            artists.append(artist)
    ...
    # plot lane center
        for key in lane_type:
            ...
            if len(lane_pts[0,:]):
                artist = ax.scatter(lane_pts[0,:],lane_pts[1,:],color='k',marker=".",s=20,label=f"lane center: {key}")
                artists.append(artist)
    ...
    if len(controlled_lanes_id) and controlled_lane:
        for controlled_lane_id in controlled_lanes_id:
            ...
            if(len(controlled_lanes_pts)):
                artist = ax.scatter(roadgraph_xyz[controlled_lanes_pts,0],roadgraph_xyz[controlled_lanes_pts,1],color='g',marker=".",s=20,label=f"controlled lane points")
                artists.append(artist)
    if return_artists:
        return ax, artists
    return ax